...
interface UseSessionContextUsageInput {
...
function getStatusFromPercent(percent: number): ContextStatus {
  if (percent >= FALLBACK_COMPACT_THRESHOLD) {
    return "compact"
  }
  if (percent >= FALLBACK_WARN_THRESHOLD) {
    return "warn"
  }
  return "ok"
}
...
function getFallbackContextNumbers(
  ...
  return {
    ...
    status,
    shouldCompact,
    tokensToCompact: shouldCompact
      ? Math.max(0, usedTokens - maxTokens * 0.6)
      : 0
  }
}
