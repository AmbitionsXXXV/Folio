...
export interface UseChatSessionsConfig {
...
export function useChatSessions(
  ...
  const fetchChatSessions = useCallback(async (): Promise<
    ChatSessionSummary[]
  > => {
  ...
  const selectInitialChatIfNeeded = useCallback(
    (chatList: ChatSessionSummary[], currentSelectedChatId: string | null) => {
      if (currentSelectedChatId || chatList.length === 0) {
        return
      }
  ...
  const deleteEmptyChat = useCallback(
    async (chatId: string): Promise<boolean> => {
      try {
        const response = await fetch(
          `${getServerUrl()}/api/ai/chat/${chatId}/empty`,
          {
            method: "DELETE",
            credentials: "include"
          }
