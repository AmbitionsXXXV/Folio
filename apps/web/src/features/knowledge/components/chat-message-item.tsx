...
const WebSearchReferences = memo(
  ({
    results,
    onViewAll
  }: {
...
export const WaitingIndicator = memo(() => {
  const { t } = useTranslation()
...
const ToolCardsSection = memo(
  ({ messageId, parts }: { messageId: string; parts: ToolMessagePart[] }) => {
    return (
      <Message from="assistant">
        <MessageContent>
