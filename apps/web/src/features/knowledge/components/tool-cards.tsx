...
interface GeneratedImageFromTool {
  url: string
  mediaType: string
}
...
function parseStockTrendOutput(value: unknown): StockTrendOutput | null {
  ...
  const periodChangePercent =
    typeof value.periodChangePercent === "number"
      ? value.periodChangePercent
      : null
  const dataPointsRaw = Array.isArray(value.dataPoints)
    ? value.dataPoints
    : null
  ...
  const dataPoints = parsedDataPoints
    ? parsedDataPoints.filter(
        (point): point is StockDataPoint => point !== null
      )
    : null
...
const ToolCardContainer = memo(
  ({ children, className }: ToolCardContainerProps) => {
    return (
      <div
        className={cn(
...
interface WeatherToolCardProps {
  part: UIMessagePart
}

export const WeatherToolCard = memo(({ part }: WeatherToolCardProps) => {
    ...
    return (
      <ToolCardContainer>
        <div className="font-medium">
          {t("knowledge.toolCards.weather.title")}
        </div>
...
interface StockToolCardProps {
  part: UIMessagePart
}

export const StockToolCard = memo(({ part }: StockToolCardProps) => {
  const { t } = useTranslation()
  ...
  if (state === "input-available") {
    ...
    return (
      <ToolCardContainer>
        <div className="font-medium">
          {t("knowledge.toolCards.stock.title")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
...
interface StockTrendToolCardProps {
  part: UIMessagePart
}

export const StockTrendToolCard = memo(({ part }: StockTrendToolCardProps) => {
  const { t } = useTranslation()
...
export const WebSearchToolCard = memo(
  ({ webSearchData, onOpenPanel }: WebSearchToolCardProps) => {
    return (
      <WebSearchCompactBar
