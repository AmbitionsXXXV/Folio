...
const SearchResultItem = memo(({ result }: { result: WebSearchResult }) => {
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm leading-snug font-medium text-foreground group-hover:text-primary">
            {result.title}
          </div>
      ...
      {snippet ? (
        <div className="mt-1.5 pl-6.5 text-xs leading-relaxed text-muted-foreground">
          {snippet}
        </div>
      ) : null}
    </a>
...
export const WebSearchPanel = memo(
  ({ open, onOpenChange, data }: WebSearchPanelProps) => {
