...
export interface JapaneseSentenceToken {
  surface: string
  reading: string
  romaji: string
...
export const JAPANESE_POS_SWATCH_COLOR_MAP: Record<
...
export interface JapaneseVocabularyItem {
  id: string
  term: string
  reading: string
  meaning: string
...
export interface JapaneseTypingExercise {
  id: string
  level: string
  scene: string
  prompt: string
...
export const isAnswerCorrect = (
  ...
  return (
    normalizedTarget.length > 0 && matchedLength === normalizedTarget.length
  )
}
...
export const buildJapaneseVocabularyBank = (
  ...
  return [...vocabularyByTerm.values()]
}
