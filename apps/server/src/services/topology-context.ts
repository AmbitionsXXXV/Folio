...
interface TopologyContextResult {
  contextText: string
...
function formatNeighborsContext(
  ...
  for (const neighbor of neighbors.values()) {
    ...
    const block = formatNeighborBlock(
      neighbor,
      neighborTags.get(neighbor.id) ?? []
    )
    if (usedChars + block.length > MAX_TOPOLOGY_CONTEXT_CHARS) {
      break
    }
