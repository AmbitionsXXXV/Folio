...
export type EnsureAttachmentImageCaptionInput = {
	userId: string
	attachmentId: string
...
export type EnsureEntryImageCaptionsInput = {
	userId: string
	entryIds: string[]
	credential?: DecryptedCredential
	model?: string
...
export type EnsureAttachmentImageCaptionResult = {
	attachmentId: string
	description: string
	modelId: string
	generatedAt: Date
...
export async function ensureAttachmentImageCaption(
	...
	const credential = resolveCaptionCredential(input.credential, allowEnvFallback)
	if (!credential) {
		log.debug(
	...
	try {
		...
		log.warn(`Failed to generate caption for attachment ${attachment.id}:`, error)
		return null
	}
}
