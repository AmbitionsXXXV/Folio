...
* [x] 认证流程（登录/注册/退出）
* [x] 快速文本捕获（Inbox 创建 entry）
* [x] 收件箱列表（分页/刷新）
* [x] 今日视图（展示今日 due/待复习数量与入口）
* [x] 复习流程（至少支持 `good`/`again` 两档）

Done: 实现了 iOS 端完整的基础功能闭环：

1. **认证流程**：复用已有的 SignIn/SignUp 组件，支持邮箱密码登录注册，退出登录功能
2. **快速文本捕获**：在 Inbox 页面顶部添加快速捕获输入框，支持一键创建新条目
3. **收件箱列表**：使用 FlatList 实现收件箱条目列表，支持下拉刷新
4. **今日视图**：展示今日到期数量、过期数量、已完成数量、新条目数量，以及连续复习天数
5. **复习流程**：支持四档评分（Again/Hard/Good/Easy），复习模式选择（Due/New/Starred/All），复习卡片展示
