...
import { AI_PROVIDERS, PROMPT_VERSIONS, PROVIDER_CONFIGS } from "@folionote/ai"
 ...
 */
const getPromptVersions = publicProcedure.handler(() => PROMPT_VERSIONS)
...
const generateText = protectedProcedure
  ...
  .handler(async ({ context, input }) => {
    ...
    if (input.model) {
      const modelListProviderId =
        API_TO_MODEL_LIST_PROVIDER[provider] || provider
