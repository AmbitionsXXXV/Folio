...
async function fetchSharedTagEdges(entryIds: string[]): Promise<GraphEdge[]> {
  ...
  const tagToEntries = new Map<string, { entryId: string; tagName: string }[]>()
  ...
  for (const [, entriesForTag] of tagToEntries) {
    ...
    for (let i = 0; i < entriesForTag.length; i++) {
      for (let j = i + 1; j < entriesForTag.length; j++) {
        ...
        const key = [a.entryId, b.entryId].toSorted().join(":")
        if (seen.has(key)) {
          continue
        }
        seen.add(key)
...
async function fetchSharedSourceEdges(
  entryIds: string[]
): Promise<GraphEdge[]> {
  if (entryIds.length === 0) {
    return []
    ...
    for (const edge of edges) {
      const key =
        edge.linkType === "ref" || edge.linkType === "manual"
          ? edge.id
          : `${edge.linkType}:${[edge.source, edge.target].toSorted().join(":")}`
      if (seen.has(key)) {
        continue
      }
      seen.add(key)
...
const getGraph = protectedProcedure
  ...
  .handler(async ({ context, input }) => {
    ...
    const edgeSources: GraphEdge[][] = [
      await fetchExplicitEdges(userId, entryIds)
    ]
...
async function expandFrontierByTags(
  ...
  for (const row of sharedTagEntries) {
    if (!visited.has(row.entryId)) {
      nextFrontier.add(row.entryId)
    }
  }
}
...
async function bfsTraverse(
  ...
  for (let hop = 0; hop < depth && frontier.length > 0; hop++) {
    ...
    for (const link of links) {
      if (!visited.has(link.sourceEntryId)) {
        nextFrontier.add(link.sourceEntryId)
      }
      if (!visited.has(link.targetEntryId)) {
        nextFrontier.add(link.targetEntryId)
      }
    }
...
const removeLink = protectedProcedure
  ...
  .handler(async ({ context, input }) => {
    ...
    const result = await db
      .delete(entryLinks)
      .where(
        and(eq(entryLinks.id, input.linkId), eq(entryLinks.userId, userId))
      )
      .returning()
