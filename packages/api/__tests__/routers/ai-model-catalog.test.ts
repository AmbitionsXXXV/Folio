...
describe("Model Catalog Merge Logic", () => {
  ...
  describe("mergeModelSettings", () => {
    it("should use default enabled value when no user override exists", () => {
      ...
      expect(
        result.find((m) => m.id === "text-embedding-3-small")?.enabled
      ).toBe(false)
    })
    ...
    it("should handle multiple user overrides", () => {
      ...
      expect(
        result.find((m) => m.id === "claude-sonnet-4-20250514")?.enabled
      ).toBe(true)
    })
    ...
    it("should distinguish models by type", () => {
      ...
      const chatModel = result.find(
        (m) => m.id === "model-x" && m.type === "chat"
      )
  ...
  describe("validateModelExists", () => {
    it("should return true for existing model", () => {
      expect(
        validateModelExists(defaultModels, "openai", "gpt-4o", "chat")
      ).toBe(true)
    })
    ...
    it("should return false for wrong provider", () => {
      expect(
        validateModelExists(defaultModels, "anthropic", "gpt-4o", "chat")
      ).toBe(false)
    })
  })
})
