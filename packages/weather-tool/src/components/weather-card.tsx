...
function getWeatherType(
	...
	if (lower.includes('rain') || lower.includes('雨') || lower.includes('shower'))
		return 'rainy'
	if (lower.includes('snow') || lower.includes('雪')) return 'snowy'
	if (lower.includes('storm') || lower.includes('thunder') || lower.includes('雷'))
		return 'stormy'
	if (
...
function RainDrops() {
	...
	return (
		<div className="pointer-events-none absolute inset-0 overflow-hidden rounded-xl">
			{rainDrops.map((drop) => (
				<div
					className="absolute h-4 w-0.5 animate-rain rounded-full bg-linear-to-b from-blue-400/60 to-transparent"
					key={drop.id}
					style={{
						left: drop.left,
...
// 雪花动画组件
function Snowflakes() {
	const snowflakes = Array.from({ length: snowflakeCount }, (_, flakeIndex) => ({
		id: `snow-${flakeIndex}`,
	...
	return (
		<div className="pointer-events-none absolute inset-0 overflow-hidden rounded-xl">
			{snowflakes.map((flake) => (
				<div
					className="absolute h-1.5 w-1.5 animate-snow rounded-full bg-white/70 shadow-sm"
					key={flake.id}
					style={{
						left: flake.left,
	...
	return (
		<div className="pointer-events-none absolute -top-4 -right-4">
			<div className="relative h-16 w-16">
				...
				{sunRays.map((ray) => (
					<div
						className="absolute top-1/2 left-1/2 h-8 w-0.5 origin-bottom animate-ray-spin bg-linear-to-t from-yellow-300/40 to-transparent"
						key={ray.id}
						style={{
...
function Clouds() {
	return (
		<div className="pointer-events-none absolute inset-0 overflow-hidden rounded-xl">
			<div className="absolute top-2 -right-2 h-6 w-12 animate-cloud-drift rounded-full bg-linear-to-r from-slate-300/30 to-slate-200/20 blur-[2px]" />
			<div
				className="absolute top-4 right-8 h-4 w-8 animate-cloud-drift rounded-full bg-linear-to-r from-slate-300/20 to-slate-200/10 blur-[1px]"
...
function Lightning() {
	return (
		<div className="pointer-events-none absolute inset-0 overflow-hidden rounded-xl">
			<div className="absolute top-2 right-4 animate-lightning text-yellow-300/70">
				<svg
			...
			<div className="relative z-10 grid gap-3 text-xs">
				<div className="flex items-center justify-between gap-2">
					<div className="min-w-0">
						<div className="text-balance font-semibold text-foreground">{title}</div>
						<div className="truncate text-muted-foreground text-xs">{location}</div>
					</div>
				</div>

				<div className="inline-flex items-center gap-1.5 text-pretty text-muted-foreground text-xs">
				...
				<div className="grid grid-cols-2 gap-2">
					<div className="col-span-2 rounded-lg bg-background/60 px-3 py-2 shadow-sm backdrop-blur-sm">
						...
						<div className="font-[tabular-nums] font-bold text-2xl text-foreground">
							{temperatureValue}
						</div>
					</div>
